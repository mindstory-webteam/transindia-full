...
const PAGE_STYLES = `
  .comp-export-btn { transition: background .15s, border-color .15s, opacity .15s; }
  .comp-export-btn:hover:not(:disabled) { background:#F4F7FB; border-color:#CBD5E1; }
  .comp-export-btn:disabled { opacity:.5; cursor:not-allowed; }
`;
...
export default function ComplaintPage() {
  ...
  const exportExcel = () => {
    if (!filtered.length) { toast.error("No complaints to export"); return; }

    const headers = ["Date", "Name", "Policy / Ref", "Category", "Mobile", "Details", "Status"];
    const aoa = [headers];
    ...
    XLSX.writeFile(wb, `complaints-${fileStamp()}.xlsx`);
    toast.success(`Exported ${filtered.length} complaint(s)`);
  };
  ...
  const exportPDF = () => {
    ...
    doc.text("Complaints", left, 38);
    doc.setFontSize(9);
    doc.setTextColor(120, 120, 120);
    doc.text(
      `Exported ${new Date().toLocaleString("en-IN")}   •   ${filtered.length} complaint(s)` +
    ...
    const head = [["Date", "Name", "Policy / Ref", "Category", "Mobile", "Details", "Status"]];
    ...
    doc.save(`complaints-${fileStamp()}.pdf`);
    toast.success(`Exported ${filtered.length} complaint(s)`);
  };
    ...
    <div>
      ...
      <div style={{ background: "linear-gradient(120deg, rgb(255, 244, 240) 0%, rgb(255, 255, 255) 58%)", border: "1px solid rgb(251, 224, 216)", borderRadius: "18px", padding: "22px 24px", marginBottom: "28px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "16px", flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", flex: "1 1 200px", justifyContent: "flex-end", alignItems: "center" }}>
          ...
          <button className="comp-export-btn" onClick={exportExcel} disabled={!filtered.length} style={exportBtnStyle} title="Download as Excel">
              ...
              {currentComplaints.map((c, i) => (
                <tr key={c._id} style={{ borderBottom: i < currentComplaints.length-1 ? "1px solid var(--border)" : "none" }}>
                  ...
                  <td style={{ padding:"12px 16px", color:"#475569" }}>
                    {c.details ? (
                      <button 
                        onClick={() => setModalData({ title: "Complaint Details", text: c.details })}
